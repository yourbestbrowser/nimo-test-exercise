...
```bash
nimo-exercise$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
nimo-exercise$ sam local start-api
nimo-exercise$ curl http://localhost:3000/
```
...
```bash
nimo-exercise$ sam logs -n HelloWorldFunction --stack-name nimo-exercise --tail
```
...
```bash
sam delete --stack-name nimo-exercise
```
